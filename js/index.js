...
function init() {
    ...
    dragPoints.push(new DragPoint((canvas.width*0), canvas.height*0.5));
    dragPoints.push(new DragPoint((canvas.width*1)+25, canvas.height*0.5));

    var baseNoiseOpts      = { base: 100000, amplitude: 0.6, speed: 0 };
...
function mouseUp(e) { //Leaves drag
    for (var i = 0, len = dragPoints.length; i < len; i++) {
...
function drawLightningBlur(line, blur, maxSize) {
    ...
    line.eachPoints(function(p, i, len) {
        ...
        context.arc(p.x, p.y, 1*(high/4), 10, Math.PI * 2, false);
    });
    context.fill();
    context.restore();
}
...
DragPoint.prototype = extend({}, Point.prototype, {
    ...
    update: function(mouse) {
        ...
        this._currentRadius *= randomRange(0.5, 1);
    },
